...
> **Corporate-minimal scope (2026-05-19).** The Yambr fork ships in corporate-minimal configuration: billing (Stripe) and referral surfaces are gated off at build time and UI-hidden (see commit `c4d2ca5e` + `.planning/phases/09-client-e2e-tests/CLIENT-CUTS.md`). Previously-documented endpoints `/api/stripe/{checkout,portal,switch-plan,preview-switch}` and `/api/referrals/{stats,invite,invites}` are deferred to a future v2 fork variant and intentionally absent from this spec. Self-hosters can ignore them entirely.
