...
> **Default build is corporate-minimal (2026-05-08 pivot, amended 2026-05-09 by Phase 05).** `npm run build` (no env vars) produces a *minimal corporate-self-hosted* binary, NOT an upstream-parity binary. Two feature flags default OFF — Stripe billing UI and the referral program — and only dictation, transcription (whisper.cpp + OpenAI Whisper file mode), reasoning (multi-provider via runtime BYOK API keys), and Google / Microsoft / Apple OAuth ship by default. **Self-hosters whose backend does NOT implement `/api/stripe/*` or `/api/referrals/*` should ship the default build — the client will not call those endpoints.** To recreate upstream-parity behavior, see the *Upstream Parity Build* section below and `docs/BUILD_CONFIG.md` for the full variable reference.
>
> **Realtime streaming default (Phase 05, 2026-05-09):** `OPENWHISPR_STREAMING` defaults to `true`. Realtime ASR routes through the corporate backend's `WSS /v1/realtime` endpoint (Speaches+LiteLLM) rather than direct third-party WebSockets (AssemblyAI / Deepgram). The build var `OPENWHISPR_REALTIME_WSS_URL` derives automatically from `OPENWHISPR_BACKEND_URL`. Set `OPENWHISPR_STREAMING=false` if your backend has not yet deployed the realtime relay. A B1 auto-disable safety net catches the default-build-with-no-backend case (no `OPENWHISPR_BACKEND_URL` set) so it cannot ship a crashing binary — streaming is forced off when no realtime URL can be derived.
